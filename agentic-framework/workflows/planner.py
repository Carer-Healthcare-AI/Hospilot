...
AVAILABLE_AGENTS = [
    ...
    {"id": "bed_prediction_agent", "label": "Bed Prediction",      "color": "#0284c7",
     "description": "owns forecasting bed availability 4-24h out (forecast only, no placement)"},
...
SUB_AGENTS: dict[str, list[SubAgent]] = {
    'bed_agent': [
        ...
        SubAgent('sa_bed_prediction', 'Bed Prediction', [
            Task('ta_predict_icu_saturation', 'Derive ICU saturation risk score from bed census', ['saturation_pct', 'risk']),
            Task('ta_generate_capacity_alert', 'Generate an alert when predicted occupancy exceeds threshold', ['alert_sent']),
            Task('ta_trigger_surge_forecast', 'Trigger a surge forecast when ER inflow is rising', ['forecast']),
            Task('ta_recommend_overflow_strategy', 'Reason over census and produce redistribution plan', ['strategy']),
            Task('ta_predict_discharge_probability', 'Predict probability each ICU patient will discharge soon', ['predictions']),
            Task('ta_notify_discharge_team', 'Notify discharge team of high-probability discharge patients', ['notified']),
            Task('ta_trigger_clearance_workflow', 'Trigger billing and pharmacy clearance for a discharge', ['triggered']),
            Task('ta_predict_discharge_horizon', 'Forecast time-to-next-discharge for bed capacity planning', ['horizon_minutes']),
            Task('ta_run_surge_model', 'Run ER admission surge demand model', ['demand_forecast']),
            Task('ta_alert_operations_team', 'Alert the operations team of a surge prediction', ['notified']),
            Task('ta_notify_staffing_agent', 'Notify staffing agent of predicted surge demand', ['notified']),
            Task('ta_recommend_overflow_zone', 'Recommend a temporary expansion or overflow zone', ['recommendation']),
        ], 'Analyses current bed usage and predicts capacity pressures over the next 4–24 hours'),
        ...
        SubAgent('sa_bed_pred_forecast', 'Capacity Forecast', [
            Task('ta_run_capacity_forecast', 'Generate plain-language capacity forecast and risk classification', ['forecast', 'risk_level']),
        ], 'Generates plain-language capacity forecast and risk classification with Claude'),
        SubAgent('sa_bed_turnover', 'Bed Turnover Forecast', [
            Task('ta_forecast_bed_turnover', 'Forecast of beds becoming available in the next shift, per ward, from occupancy, discharge outlook, cleaning backlog and ER inflow', ['forecast_available', 'wards_forecast', 'low_capacity_count', 'wards']),
        ...
        SubAgent('sa_bed_occupancy', 'Bed Occupancy Forecast', [
            Task('ta_forecast_bed_occupancy', 'Whole-hospital predicted bed census and overflow risk at a horizon derived from the goal, from current occupancy, expected ER admissions and discharge outlook', ['forecast_available', 'predicted_occupancy_percent', 'overflow_risk', 'recommended_action']),
        ...
        SubAgent('sa_bed_ward_capacity', 'Ward Capacity Forecast', [
            Task('ta_forecast_bed_ward_capacity', 'Per-ward forecast of occupancy and capacity utilization at a horizon derived from the goal, with predicted available beds, net patient change and overflow risk, from each ward''s beds, discharge outlook and ER pressure', ['forecast_available', 'wards_forecast', 'high_risk_count', 'wards']),
        ...
        SubAgent('sa_bed_isolation_demand', 'Isolation Bed Demand Forecast', [
            Task('ta_forecast_bed_isolation_demand', 'Forecast isolation-bed requirements and shortage risk at a horizon derived from the goal, from current isolation beds occupied/total, active infectious-disease cases and suspected cases', ['forecast_available', 'predicted_isolation_beds_required', 'isolation_shortage_risk', 'recommended_action']),
    ...
    'icu_agent': [
        ...
        SubAgent('sa_icu_capacity_forecast', 'ICU Demand Forecast', [
            Task('ta_forecast_icu_demand', 'Forecast expected ICU admissions over the next 24 hours, with a capacity alert', ['forecast_available', 'predicted_admissions_24h', 'capacity_alert']),
        ...
        SubAgent('sa_icu_occupancy', 'ICU Occupancy Forecast', [
            Task('ta_forecast_icu_occupancy', 'Forecast ICU census (occupied and free beds) and overflow risk at a horizon derived from the goal, from current ICU occupancy and critical patients awaiting ICU', ['forecast_available', 'predicted_occupancy_percent', 'overflow_risk', 'recommended_action']),
        ...
        SubAgent('sa_icu_stepdown_demand', 'Step-down Demand Forecast', [
            Task('ta_forecast_icu_stepdown_demand', 'Forecast ICU patients becoming ready for a ward (step-down) bed over a horizon derived from the goal, with a ward-absorption status and recommended action, from ICU census, available ward beds, ward occupancy and ward staffing', ['forecast_available', 'predicted_step_down_bed_demand', 'ward_status', 'recommended_action']),
        ...
        SubAgent('sa_icu_ventilator_demand', 'Ventilator Demand Forecast', [
            Task('ta_forecast_icu_ventilator_demand', 'Forecast ventilators clinically required over a horizon derived from the goal, including demand the current free fleet cannot meet, with a recommended action, from patients currently on ventilators, ventilator-capable beds free and ICU census', ['forecast_available', 'predicted_ventilator_demand', 'unmet_ventilator_need', 'recommended_action']),
        ...
        SubAgent('sa_icu_los', 'ICU LOS Forecast', [
            Task('ta_forecast_icu_los', 'Forecast the average ICU length of stay (days) at a horizon derived from the goal, with the trend vs the current average and a recommended action, from current mean LOS, ICU census, ventilated patients and bed occupancy', ['forecast_available', 'predicted_average_los', 'los_trend', 'recommended_action']),
        ...
        SubAgent('sa_icu_staffing_demand', 'ICU Staffing Demand Forecast', [
            Task('ta_forecast_icu_staffing_demand', 'Forecast the peak acuity-weighted ICU NURSES required over a horizon derived from the goal, plus hours likely short-staffed, with a recommended action, from ICU census, ventilated patients, nurses rostered and average LOS', ['forecast_available', 'predicted_peak_nurses_required', 'hours_likely_short_staffed', 'recommended_action']),
    ...
    'er_agent': [
        ...
        SubAgent('sa_er_surge_prediction', 'Surge Predictor', [
            Task('ta_forecast_er_surge', 'Forecast of ER arrival volume per hour over the next few hours, tagging each hour normal/elevated/surge', ['forecast_available', 'total_expected', 'peak_volume', 'surge_hour_count', 'hours']),
        ...
        SubAgent('sa_er_wait_time', 'Wait Time Forecast', [
            Task('ta_forecast_er_wait_time', 'Forecast the average ER wait time (minutes) a chosen time ahead derived from the goal, with wait status and 8-minute target-breach risk, from the waiting queue, patients in the ED, recent arrivals and doctor/nurse staffing', ['forecast_available', 'predicted_wait_minutes', 'wait_status', 'target_breach_risk', 'recommended_action']),
        ...
        SubAgent('sa_er_boarding_forecast', 'Boarding Forecast', [
            Task('ta_forecast_er_boarding', 'Forecast admitted patients boarding in the ED awaiting inpatient beds over a horizon derived from the goal, with predicted boarding time and risk, from current boarders, available inpatient beds and hospital census', ['forecast_available', 'predicted_boarding_patients', 'boarding_status', 'boarding_risk', 'recommended_action']),
        ...
        SubAgent('sa_er_lwbs', 'LWBS Forecast', [
            Task('ta_forecast_er_lwbs', 'Forecast patients Leaving Without Being Seen (LWBS) over a horizon derived from the goal, with LWBS rate and risk, from the waiting queue, average wait time and ED load', ['forecast_available', 'predicted_lwbs_patients', 'predicted_lwbs_rate', 'lwbs_risk_level', 'recommended_action']),
        ], 'Forward-looking forecast of ED patients who will LEAVE WITHOUT BEING SEEN (LWBS) over a horizon inferred from the request (6h-7d), with LWBS rate and risk. Include when the goal asks about LWBS, walkouts, left-without-treatment, or wait-driven attrition risk. Distinct from sa_er_wait_time (predicts wait minutes) and sa_er_surge_prediction (arrival volume).'),
        SubAgent('sa_er_congestion', 'Congestion Forecast', [
            Task('ta_forecast_er_congestion', 'Forecast overall ED congestion as a composite score (0-100) over a horizon derived from the goal, with congestion level and recommended action, from ED census, the waiting queue, boarders, recent arrivals, critical patient load and bed/staffing availability', ['forecast_available', 'predicted_congestion_score', 'congestion_level', 'recommended_action']),
        ...
        SubAgent('sa_er_ambulance_arrivals', 'Ambulance Arrival Forecast', [
            Task('ta_forecast_ambulance_arrivals', 'Forecast how many ambulances will arrive at the ED over a horizon derived from the goal, with a recommended action, from recent ambulance activity (live fleet) and ED load', ['forecast_available', 'predicted_ambulance_arrivals', 'arrival_level', 'recommended_action']),
    ...
    'staff_agent': [
        ...
        SubAgent('sa_nurse_demand', 'Nurse Demand Forecast', [
            Task('ta_forecast_nurse_demand', 'Forecast required nurses over a horizon derived from the goal, with the staffing gap and patient-care risk, from census, patient acuity, current nurses on duty, and ER/ICU/discharge load', ['forecast_available', 'predicted_required_nurses', 'staffing_gap', 'patient_care_risk', 'recommended_action']),
        ...
        SubAgent('sa_doctor_demand', 'Doctor Demand Forecast', [
            Task('ta_forecast_doctor_demand', 'Forecast required physicians over a horizon derived from the goal, with the staffing gap and clinical-capacity risk, from census, patient acuity, doctors on duty, and ER/ICU/critical/discharge workload', ['forecast_available', 'predicted_required_doctors', 'staffing_gap', 'clinical_capacity_risk', 'recommended_action']),
        ...
        SubAgent('sa_shift_coverage', 'Shift Coverage Forecast', [
            Task('ta_forecast_shift_coverage', 'Forecast the total staff (nursing, medical, support) needed to safely cover the shift over a horizon derived from the goal, with the staffing gap and a recommended action, from census, acuity, scheduled staff and ER/ICU/surgery load', ['forecast_available', 'predicted_staff_required', 'staffing_gap', 'recommended_action']),
        ...
        SubAgent('sa_overtime_forecast', 'Overtime Forecast', [
            Task('ta_forecast_overtime', 'Forecast staff overtime hours over a horizon derived from the goal, with a recommended action, from census, scheduled staff, acuity and ER/ICU/surgery load', ['forecast_available', 'predicted_overtime_hours', 'recommended_action']),
        ...
        SubAgent('sa_absenteeism_forecast', 'Absenteeism Forecast', [
            Task('ta_forecast_absenteeism', 'Forecast the number of staff on unplanned absence over a horizon derived from the goal, with a recommended action, from scheduled staff, census, acuity and workload', ['forecast_available', 'predicted_absent_staff', 'absent_share_pct', 'recommended_action']),
        ...
        SubAgent('sa_workforce_utilization', 'Workforce Utilization Forecast', [
            Task('ta_forecast_workforce_utilization', 'Forecast the worst-hour percent of deployable workforce capacity consumed over a horizon derived from the goal, plus critical-band hours, with a recommended action, from staff available/on-duty, census, acuity and workload', ['forecast_available', 'predicted_peak_workforce_utilization', 'predicted_critical_hours', 'recommended_action']),
        ...
        SubAgent('sa_skill_mix', 'Skill Mix Forecast', [
            Task('ta_forecast_skill_mix', 'Forecast which clinical SKILLS will be needed (concurrent headcount per specialty: ICU nurses, ER nurses, anesthesiologists, respiratory therapists, critical-care physicians, OR staff) over a horizon derived from the goal, from census, ventilated patients, ICU load and the surgical schedule', ['forecast_available', 'predicted_skill_requirements', 'recommended_action']),
    ...
    'ambulance_agent': [
        ...
        SubAgent('sa_ambulance_response', 'Response Time Forecast', [
            Task('ta_forecast_ambulance_response', 'Forecast the service-average ambulance response time (minutes) over a horizon derived from the goal, with 8-minute SLA breach risk, from available and active units in the live fleet', ['forecast_available', 'predicted_response_minutes', 'target_breach_risk', 'recommended_action']),
        ...
        SubAgent('sa_ambulance_fleet_utilization', 'Fleet Utilization Forecast', [
            Task('ta_forecast_ambulance_fleet_utilization', 'Forecast the worst-hour percent of the ambulance fleet committed over a horizon derived from the goal, plus saturated hours, with a recommended action, from total/on-mission/available units in the live fleet', ['forecast_available', 'predicted_peak_fleet_utilization', 'predicted_saturated_hours', 'recommended_action']),
        ...
        SubAgent('sa_ambulance_availability', 'Availability Forecast', [
            Task('ta_forecast_ambulance_availability', 'Forecast the minimum ambulances free for dispatch at the worst hour over a horizon derived from the goal, with a recommended action, from total/on-mission/available units in the live fleet', ['forecast_available', 'predicted_minimum_ambulances_available', 'recommended_action']),
        ], 'Forward-looking forecast of ambulance AVAILABILITY — the minimum number of ambulances that will be free for dispatch at the worst hour over a horizon inferred from the request (3h-3d). Include when the goal asks about dispatch reserve/headroom, guaranteed-available unit count, or how thin the free fleet will get. Distinct from sa_ambulance_fleet_utilization (the % view of the same model) and sa_ambulance_response (response minutes) — this is the COUNT of free units.'),
    ],
    'discharge_agent': [
        ...
        SubAgent('sa_discharge_volume', 'Discharge Volume Forecast', [
            Task('ta_forecast_discharge_volume', 'Forecast hospital-wide discharge volume over a horizon derived from the goal, with beds freed and expected completion %, from current census, the discharge outlook and the medically-cleared count', ['forecast_available', 'predicted_discharges', 'beds_freed', 'capacity_risk', 'recommended_action']),
        ...
        SubAgent('sa_discharge_delayed', 'Delayed Discharge Forecast', [
            Task('ta_forecast_delayed_discharge', 'Forecast how many discharges will be DELAYED (beds blocked) over a horizon derived from the goal, with patient-flow risk, from census, the discharge outlook, medically-ready count and free ward beds', ['forecast_available', 'predicted_delayed_discharges', 'delayed_percent', 'flow_risk', 'recommended_action']),
        ...
        SubAgent('sa_weekend_discharge', 'Weekend Discharge Forecast', [
            Task('ta_forecast_weekend_discharge', 'Forecast discharges on a weekend day and across the full weekend, derived from the goal, with a recommended action, from current census, the discharge outlook and the medically-cleared count', ['forecast_available', 'discharges_per_weekend_day', 'discharges_full_weekend', 'recommended_action']),
    ...
    'pharmacy_agent': [
        ...
        SubAgent('sa_pharmacy_queue', 'Pharmacy Queue Optimization Agent', [
            ...
            Task('ta_forecast_pharmacy_queue', 'Forecast the dispensing-counter queue length for the next hour', ['forecast_available', 'predicted_queue_length', 'queue_level']),
        ...
        SubAgent('sa_pharmacy_capacity', 'Pharmacy Capacity Prediction Agent', [
            Task('ta_fetch_dispensing_history', 'Fetch historical dispensing demand data', ['has_history', 'avg_daily_orders']),
            Task('ta_forecast_pharmacy_demand', 'ML forecast of next-day dispensing demand per drug (Hospilot /pharmacy/demand)', ['forecast_available', 'forecast_orders', 'surge_expected']),
            Task('ta_check_dispensing_capacity', 'Compare forecast to current dispensing capacity', ['capacity_gap', 'at_risk']),
            Task('ta_notify_pharmacy_surge', 'Notify Command Center if pharmacy surge is expected', ['notified']),
        ], 'Forecasts dispensing demand and alerts command center of surge risk'),
        SubAgent('sa_pharmacy_stockout', 'Stock-out Forecast', [
            Task('ta_forecast_pharmacy_stockout', 'Forecast medication stock-out events over a horizon derived from the goal, with stock-out rate, projected low-stock drugs and shortage risk, from current inventory, low-stock count and daily dispensing rate', ['forecast_available', 'predicted_stockout_events', 'predicted_low_stock_medications', 'stockout_risk', 'recommended_action']),
        ...
        SubAgent('sa_pharmacy_reorder', 'Reorder Forecast', [
            Task('ta_forecast_pharmacy_reorder', 'Forecast how many SKUs will drop below their 7-day reorder point over a horizon derived from the goal -- what purchasing should raise today -- with a recommended action, from days-of-cover, lines below reorder and per-line consumption', ['forecast_available', 'predicted_drugs_to_reorder', 'recommended_action']),
        ...
        SubAgent('sa_pharmacy_inventory', 'Inventory Forecast', [
            Task('ta_forecast_pharmacy_inventory', 'Forecast the minimum days of stock cover across a horizon derived from the goal (how thin the shelf gets at its worst), with a recommended action, from opening stock, per-line consumption and consumption variability', ['forecast_available', 'predicted_minimum_days_of_stock', 'recommended_action']),
    ...
    'lab_agent': [
        ...
        SubAgent('sa_capacity_prediction', 'Lab Capacity Prediction Agent', [
            Task('ta_get_historical_demand', 'Fetch historical workload data', ['has_history', 'avg_daily_orders']),
            Task('ta_run_workload_forecast', 'AI-powered demand forecast for next shift/day', ['forecast_orders', 'surge_expected']),
            Task('ta_forecast_analyzer_util', 'ML forecast of next-hour utilization per analyzer (Hospilot /lab/analyzer-util)', ['analyzers_forecast', 'critical_count', 'forecast_available']),
            Task('ta_check_capacity_threshold', 'Compare forecast to analyzer capacity', ['capacity_gap', 'at_risk']),
            Task('ta_surge_notify_command', 'Notify Command Center if surge is expected', ['notified']),
        ], 'Forecasts workload demand and notifies command center of surge risk'),
        SubAgent('sa_lab_test_volume', 'Test Volume Forecast', [
            Task('ta_forecast_test_volume', 'Forecast how many lab tests will be ordered over a horizon derived from the goal, with expected specimen volume, analyzer/staff utilisation and overload risk, from recent order volume, online analyzers and lab staffing', ['forecast_available', 'predicted_test_volume', 'predicted_analyzer_utilization', 'capacity_overload_risk', 'recommended_action']),
        ...
        SubAgent('sa_lab_tat', 'Lab Turnaround Forecast', [
            Task('ta_forecast_lab_tat', 'Forecast the p90 collection-to-result turnaround time (minutes) for a priority band over a horizon derived from the goal, with a recommended action, from current backlog, analyzers operational and technologist staffing', ['forecast_available', 'predicted_turnaround_minutes', 'test_priority', 'recommended_action']),
        ], 'Forward-looking forecast of lab TURNAROUND TIME — the p90 collection-to-result clock (minutes) for a priority band (STAT/Urgent/Routine/Batch) over a horizon inferred from the request (3h-3d). Include when the goal asks about predicted lab TAT, result turnaround, or SLA-clock risk over a time horizon. Distinct from sa_lab_test_volume (specimen COUNT) and the live TAT-bottleneck work — this predicts the turnaround CLOCK.'),
        SubAgent('sa_lab_critical_result', 'Critical Result Forecast', [
            Task('ta_forecast_lab_critical_result', 'Forecast critical lab results per hour needing an urgent clinician callback over a horizon derived from the goal, with a recommended action, from the expected test rate and callback staffing', ['forecast_available', 'predicted_critical_results', 'recommended_action']),
    ...
    'ot_agent': [
        ...
        SubAgent('sa_ot_utilization', 'OT Utilization Forecast', [
            Task('ta_forecast_ot_utilization', 'Forecast the % of operating-theatre capacity that will be used over a horizon derived from the goal, with used/free OT hours and capacity risk, from open theatres and the scheduled/emergency case load', ['forecast_available', 'predicted_utilization_percent', 'capacity_risk', 'recommended_action']),
        ...
        SubAgent('sa_ot_surgery_volume', 'Surgery Volume Forecast', [
            Task('ta_forecast_ot_surgery_volume', 'Forecast how many surgeries will be performed over a horizon derived from the goal (total, emergency and elective), with capacity risk, from operating rooms and the scheduled/emergency case load', ['forecast_available', 'predicted_surgeries', 'predicted_emergency_surgeries', 'capacity_risk', 'recommended_action']),
        ...
        SubAgent('sa_ot_turnaround_forecast', 'OT Turnaround Forecast', [
            Task('ta_forecast_ot_turnaround', 'Forecast the average operating-theatre turnaround time (minutes) at a horizon derived from the goal, with the change vs the current average and a recommended action, from the current turnaround, completed surgeries and open/in-use theatres', ['forecast_available', 'predicted_average_turnaround_time', 'change_vs_now_minutes', 'recommended_action']),
        ...
        SubAgent('sa_ot_surgery_duration', 'Surgery Duration Forecast', [
            Task('ta_forecast_ot_surgery_duration', 'Forecast the mean surgery duration (minutes) over a horizon derived from the goal, with the change vs the current average and a recommended action, from current average duration, scheduled/emergency/elective cases and open theatres', ['forecast_available', 'predicted_average_duration', 'change_vs_now_minutes', 'recommended_action']),
        ...
        SubAgent('sa_ot_emergency_demand', 'Emergency OT Demand Forecast', [
            Task('ta_forecast_ot_emergency_demand', 'Forecast emergency surgeries requiring immediate OT access over a horizon derived from the goal, with a recommended action, from current emergency cases, ER critical patients, ambulance arrivals, emergency admissions and open theatres', ['forecast_available', 'predicted_emergency_surgeries', 'operating_rooms_open', 'recommended_action']),
        ...
        SubAgent('sa_ot_equipment_utilization', 'Equipment Utilization Forecast', [
            Task('ta_forecast_ot_equipment_utilization', 'Forecast the peak share of critical theatre EQUIPMENT committed over a horizon derived from the goal, with a recommended action, from equipment committed to active cases, scheduled/emergency surgeries and theatre utilization (equipment inventory is assumed — no inventory source)', ['forecast_available', 'predicted_peak_equipment_utilization', 'equipment_data_assumed', 'recommended_action']),
    ...
    'revenue_agent': [
        ...
        SubAgent('sa_rev_forecast', 'Revenue Forecast', [
            Task('ta_forecast_revenue', 'Forecast total hospital revenue (INR) over a horizon derived from the goal, with per-patient revenue and risk, from average invoice value and today''s admissions / ER / surgery / lab activity', ['forecast_available', 'predicted_revenue', 'revenue_per_patient', 'revenue_risk', 'recommended_action']),
        ...
        SubAgent('sa_rev_claim_denial', 'Claim Denial Forecast', [
            Task('ta_forecast_claim_denial', 'Forecast the insurance claim denial RATE (%) and denied-value rate over a horizon derived from the goal, with a denial status and recommended action, from claims submitted (7d), the trailing-30d denial rate, average claim amount, high-value claims and processing staff', ['forecast_available', 'predicted_denial_rate', 'predicted_denied_value_rate', 'denial_status', 'recommended_action']),
        ...
        SubAgent('sa_rev_claim_volume', 'Claim Volume Forecast', [
            Task('ta_forecast_claim_volume', 'Forecast how many insurance claims will be submitted over a horizon derived from the goal, with a per-staff load status and recommended action, from recent discharges, completed billing cases, pending claims, high-value claims and processing staff', ['forecast_available', 'predicted_claim_volume', 'load_status', 'recommended_action']),
    ...
    'billing_agent': [
        ...
        SubAgent('sa_billing_backlog', 'Pending Invoice Forecast', [
            Task('ta_forecast_pending_invoices', 'Forecast the billing backlog (pending invoice count + value in INR) over a horizon derived from the goal, with backlog risk, from current pending invoices, invoices created today, avg invoice value, pending claims and today''s discharges', ['forecast_available', 'predicted_pending_invoices', 'predicted_pending_amount', 'backlog_risk', 'recommended_action']),
        ...
        SubAgent('sa_billing_delay', 'Billing Delay Forecast', [
            Task('ta_forecast_billing_delay', 'Forecast the share of billing cases that will breach the SLA target (%) over a horizon derived from the goal, with delay severity, from pending cases, new cases today, currently delayed cases, billing staff, pending claims and high-value bills', ['forecast_available', 'predicted_sla_breach_share_pct', 'delay_severity', 'recommended_action']),
        ...
        SubAgent('sa_billing_workload', 'Billing Workload Forecast', [
            Task('ta_forecast_billing_workload', 'Forecast total billing work items arriving over a horizon derived from the goal (for billing-department staffing), with a workload status and recommended action, from new/pending/completed cases, billing staff, pending claims and discharges', ['forecast_available', 'predicted_workload', 'workload_status', 'recommended_action']),
